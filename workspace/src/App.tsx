  ...
  return (
    <div className="stage-bg w-full h-full overflow-hidden select-none">
      <div className="relative w-full h-full flex items-center justify-center">
        ...
        {/* planning panel — pinned to the top, below the HUD bar */}
        {ui.mode === "plan" && (
          <div
            className="safe-area absolute z-20 flex flex-col items-stretch gap-2 w-[112px] top-16 right-2
              sm:w-auto sm:flex-row sm:items-center sm:gap-3 sm:px-2"
          >
            <div className="panel chamfer px-2 py-1.5 text-center sm:text-left sm:px-4 sm:py-2">
              ...
              {ui.online && ui.oppLocked && (
                <div className="text-[8px] sm:text-[10px] font-bold text-[#3fd46d] mt-0.5">RIVAL LOCKED ✓</div>
              )}
              {ui.online && ui.awaiting && (
                <div className="text-[8px] sm:text-[10px] font-bold text-[#ffd23f] flight-flash mt-0.5">SENT — SYNCING…</div>
              )}
            </div>
            {ui.online && (ui.awaiting || ui.lockedIn) ? (
              <div className="flex flex-col items-center justify-center px-2 py-2 sm:px-4 panel chamfer-sm">
                <span className="font-disp text-lg text-[#ffd23f] flight-flash">● ● ●</span>
                <span className="text-[8px] sm:text-[9px] tracking-[0.12em] sm:tracking-[0.16em] text-white/55 font-bold text-center">
                  {ui.oppLocked
                    ? "CLASHING…"
                    : ui.myTeam === 1
                      ? "WAITING FOR CLASH"
            ...
            <div className="flex flex-col gap-1.5 justify-center">
              <button onClick={() => eng().uiLock()} disabled={ui.awaiting}
                className="btn-blade font-disp text-sm sm:text-base px-3 py-2 sm:px-7 text-[#04121c] disabled:opacity-40" style={{ background: "#ffd23f" }}>
                {ui.awaiting ? "SENT ✓" : "LOCK IN ▸"}
              </button>
              <button onClick={() => eng().uiClearPlan()} disabled={ui.awaiting}
                className="btn-blade font-disp text-[11px] px-3 py-1.5 sm:px-7 text-white/75 bg-[#12384c] disabled:opacity-40">
                CLEAR
              </button>
            </div>
          </div>
